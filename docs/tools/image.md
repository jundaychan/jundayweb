...
|------|-------|------|-----------|---------|-------|
| **GPT-Image-2** | OpenAI | ChatGPT 付费 / API | ❌ | ❌ 需翻墙 | **当前最强**：写实商拍、精准文字、品牌一致性 |
...
## GPT-Image-2（OpenAI，当前最强）

**出品方**：OpenAI

**2026年4月21日发布**，是 OpenAI 第三代旗舰生图模型（继 2025年4月 gpt-image-1、2025年12月 gpt-image-1.5 之后）。发布 12 小时内登顶 Image Arena 盲测榜全部类别第一，领先第二名 **242 分**。

### 核心能力

- **业界首个"会思考的"生图模型**：先研究、规划、推理图片结构，再动笔（把 OpenAI 推理能力引入生图）
- **写实质感 + 像素级精准文字**：海报、电商主图里的中英文字几乎不出错
- **品牌一致性强**：适合做整套风格统一的产品图、商拍
- 支持上传参考图编辑

### 价格 & 可用性

- ChatGPT 与 Codex 用户可直接用；API 于 2026年5月开放
- 上传参考图按图片输入计费（约 $8/百万 token），输出不单独计费
- **需翻墙**，没有国内直连版本

### 适合谁

追求**最高画质和最准文字**、不在意翻墙的团队。对国内直连有要求的，看下面的即梦 / 通义万相。

> 数据来源：OpenAI 官方、Image Arena 榜单（2026年4月）
...
- **局部重绘**：只改图片某个部分，其他不动
- 一键扩图、去背景、抠图
- **即梦 4.0**（2026年最新）：中文理解和审美进一步增强，新增多图融合、AI 漫画等能力，正面对标 Google Nano Banana
